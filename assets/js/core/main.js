var PIXL8 = function() {
	...
	var menuHandler = function() {
		...
		$( ".js-menu-trigger" ).click( function( e ) {
			...
			$( ".site-head" ).toggleClass( "show-mobile-menu" );
		} );


		var formnav = $(".form-nav").length;

		if( formnav ) {
			var currentText = $(".form-nav .form-nav-btn").text();
			var activeText = $(".form-nav .form-nav-btn").data("active-text");
			$(".form-nav .form-nav-btn").click(function(e){
				e.preventDefault();
				var activeFilter = $(".filter-section").slideToggle();
				$(this).parent().toggleClass("active-filter");

				if( $(this).parent().hasClass("active-filter") ) {
					$(".form-nav .form-nav-btn").text( activeText );
				} else {
					$(".form-nav .form-nav-btn").text( currentText );
				}
	...
	var formHandler = function( $container ) {

		// Collapsible alert
		$( ".js-close-alert", $container ).click(function(e) {
			e.preventDefault();
			$( this ).closest( ".alert" ).slideUp();
		} );

		// File upload
		$( "input[type='file']", $container ).on("change", function(e) {
			var $me = $( this ),
				fileName = $me.val().split('\\').pop();
			if ( $me.siblings( ".file-name" ).is( "input" ) ) {
				$me.siblings( ".file-name" ).val( fileName ).parent().addClass("has-value");
			} else {
				$me.siblings( ".file-name" ).text( fileName ).parent().removeClass("has-value");
			}
		} );
		$( ".file-name.form-control", $container ).on("click", function(e) {
			$( this ).click( function() {
				$(this).siblings( ".btn-browse" ).trigger( "click" );
			} );
		} );

		$( ".form-field-input:not(.mod-label-up) .form-control", $container ).on( "change focus", function() {
			var $wrapper = $( this ).closest( ".form-field-input" );
